...
void main() {
    ...
    float center = 1.0 - abs(texCoord0.x - 0.5) * 2.0;
    center = clamp(center, 0.0, 1.0);

    float edgeDist = min(min(texCoord0.x, 1.0 - texCoord0.x), min(texCoord0.y, 1.0 - texCoord0.y));
    float fresnel = pow(1.0 - clamp(edgeDist * 4.0, 0.0, 1.0), 2.0) * 0.08;

    vec3 rgb = base.rgb * (1.0 + 0.10 * sheen) + vec3(0.02, 0.03, 0.04) * center * 0.08;
    rgb += vec3(fresnel);

    float alpha = base.a * mask * (0.76 + 0.24 * sheen);
    if (alpha < 0.003) {
        discard;
    }
