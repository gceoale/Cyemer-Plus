...
void main() {
    ...
    float centerGlow = exp(-pow((texCoord0.y - 0.5) * 2.8, 2.0));
    float sideFade = 1.0 - abs(texCoord0.x - 0.5) * 1.3;
    sideFade = clamp(sideFade, 0.0, 1.0);

    float pulse = 0.5 + 0.5 * centerGlow * sideFade;
    vec3 accent = vec3(0.06, 0.14, 0.22);
    vec3 rgb = base.rgb + accent * (0.22 + 0.38 * pulse);
