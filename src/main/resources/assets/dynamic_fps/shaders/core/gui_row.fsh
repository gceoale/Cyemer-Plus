...
void main() {
    ...
    float shadow = innerShadow(texCoord0, cornerRadius, 3.5);
    float centerGlow = exp(-pow((texCoord0.y - 0.5) * 3.0, 2.0));
    vec3 rgb = base.rgb * (0.94 + 0.06 * centerGlow);
    rgb -= vec3(0.015, 0.02, 0.025) * shadow;
